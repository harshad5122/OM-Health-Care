...
//         ...(editingMessage && { message_id: editingMessage._id })
//       };
...
//                 <p>{messages.find(m => m._id === msg.reply_to)?.message || "Original message"}</p>
//               </div>
//             )}
            
//             {msg.message_type === "text" && <p className="chat-text">{msg.message}</p>}
  ...
  const sendMessage = () => {
    ...
    if (socket && socket.connected) {
      ...
      const msgData = {
        ...
        attechment_id: attachment._id, 
      //   ...(attachment && { 
      //   attechment_id: attachment._id, 
      //   attechment_details: [attachment] 
      // }),
        created_at: new Date(),
        ...(replyingTo && { reply_to: replyingTo._id })
      };
...
//     const handleMessageDeleted = (deletedMsg) => {
//       setMessages(prev => prev.filter(msg => msg._id !== deletedMsg.messageId));
//     };
...
//       setEditingMessage(null);
//       setInput("");
//       return; // 👈 stop here (don’t append new message)
//     }
//       const msgData = {
//         sender_id: user._id,
//         receiver_id: adminId,
